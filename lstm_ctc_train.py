...
learningRate, nEpochs, batchSize = 0.001, 2, 64
...
with tf.Session(graph=graph) as session:
    ...
    print("\n\n", "TRAINING MODEL:START".center(100, "-"), sep="", end="\n\n\n")
    for epoch in range(startEpoch, nEpochs):
        ...
        for batchInput, batchTarget, batchSeqLengths in trainBatches:
            ...
            _, currLoss, currErrorRate, currlogitsMaxTest = session.run([optimizer, loss, errorRate, logitsMaxTest], feed_dict=feedDict)
        ...
        saver.save(session, path.join(logsDir, snapshot + "-" + str(epoch) + "-" + ".ckpt"))
    print("\n\n", "TRAINING MODEL:SUCCESS".center(100, "-"), sep="", end="\n\n\n")